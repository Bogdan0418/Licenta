...
import { Calendar, MapPin, X, Loader2, AlertTriangle, Star, MessageSquare, Check, History, Users, MessageSquare as ChatIcon, Heart } from 'lucide-react';
...
import { LocationCard } from '@/components/search/LocationCard'; // Asigură-te că e calea corectă!
            ...
            <div className="max-w-5xl mx-auto px-4 space-y-6">
                ...
                <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                    <div>
                        ...
                        <p className="text-zinc-400 text-sm font-light">Gestionează experiențele tale</p>
                    </div>
                    {profile && (
                ...
                <div className="flex items-center gap-6 border-b border-white/10 mb-6 px-2">
                    <button 
                        ...
                        className={`pb-3 text-sm font-medium transition-colors border-b-2 flex items-center gap-2 ${activeTab === 'bookings' ? 'border-[#C5A059] text-[#C5A059]' : 'border-transparent text-zinc-400 hover:text-white'}`}
                    >
                        <Calendar size={16} /> Rezervările Mele
                    </button>
                    <button 
                        onClick={() => setActiveTab('favorites')}
                        className={`pb-3 text-sm font-medium transition-colors border-b-2 flex items-center gap-2 ${activeTab === 'favorites' ? 'border-[#C5A059] text-[#C5A059]' : 'border-transparent text-zinc-400 hover:text-white'}`}
                    >
                        <Heart size={16} className={activeTab === 'favorites' ? 'fill-[#C5A059]' : ''} /> Locații Favorite
                    </button>
                </div>
                ...
                {activeTab === 'favorites' && (
                    <div className="animate-in fade-in duration-300">
                        {favoritesLoading ? (
                            ...
                            <div className="bg-white/5 rounded-2xl border border-white/5 py-16 text-center flex flex-col items-center gap-3">
                                <Heart size={40} className="text-zinc-600 mb-2" />
                                <h3 className="text-white font-medium text-lg">Nu ai nicio locație favorită încă</h3>
                                <p className="text-zinc-400 text-sm font-light max-w-sm">
                                    Explorează platforma și apasă pe inimioară pentru a salva aici locațiile tale preferate.
                                </p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {favorites?.map((location: any) => (
                                    <LocationCard key={location.publicId} location={location} />
                                ))}
                            </div>
                        )}
                    </div>
